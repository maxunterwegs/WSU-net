...
class SaliencyMapNet(nn.Module):
    ...
    def __init__(self, num_classes, gr=32, resnet_backbone='ResNet50', dense_config='normal', pretrained=True):
        ...
        self.pooling = CustomPooling(beta=1, r_0=5, dim=(-1, -2), mode=None)
...
class SaliencyUNet_dec(nn.Module):
    ...
    def __init__(self, num_classes, resnet_backbone='ResNet50', pretrained=True):
        ...
        self.onexone_decoder = torch.nn.Conv2d(in_channels=16, out_channels=1, kernel_size=1)
        ###########################
...
# model = SaliencyMapNet(num_classes=15)
#
# input = torch.randn(1, 3, 512, 512)
# output= model(input)
# # sm, scores = model(input)
# from torchsummary import summary
# device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
# summary(model.to(device), input_size=(3, 512, 512))
#
# print('done')
